...
class User
{
	...
	function __construct()
	{
		...
		if(isset($_REQUEST['admin_email']) && isset($_REQUEST['admin_phone']) && isset($_REQUEST['admin_username']) && isset($_REQUEST['admin_password']) && isset($_REQUEST['as_username']) && isset($_REQUEST['as_api_key']) && isset($_REQUEST['as_sender_id']))
		{
			...
			#verify username
			$verify_username = $db->query("SELECT * FROM `sms_users` WHERE `username`='$this->admin_username'");
			...
			{
				$saveAsCr      =  "INSERT INTO `sms_settings`(`as_username`,`as_key`,`as_sender_id`,`system_name`,`minbalance`,`security_2_factor`,`password_reset_type`,`default_country_code`,`email_from`,`email_from_name`,`email_protocol`,`email_smtp_host`,`email_smtp_secure`,`email_smtp_port`,`email_smtp_user`,`email_smtp_pass`) 
												VALUES('$this->as_username','$this->as_api_key','$this->as_sender_id','Oneplace SMS','10','1','Email','{$this->default_country_code}','sandbox@isolve.co.ke','Oneplace SMS','smtp','smtp.gmail.com','tls','587','sandbox@isolve.co.ke','9L0s9N@K!m')";

					if($saveAsCr_run = $db->query($saveAsCr))
					{
						# send mail to user
						$to = $this->admin_email;
