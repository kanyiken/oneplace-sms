...
class DeleteContact
{
	...
	function __construct()
	{
		...
		{
			#sanitization
			$this->contactreference     =  $db->real_escape_string($_GET['contactreference']);
			#check empty
			if(!empty($_GET['contactreference']))
			{
				if($remove_group_contact ==1)
				{
					...
					#delete Records
					$deleteContact = $db->query("DELETE FROM `sms_contacts` WHERE `id`='$this->contactreference'");
					#delete from groups
					$deleteGContact = $db->query("DELETE FROM `sms_group_members` WHERE `member`='$this->contactreference'");
