...
class AddToGroup
{
	...
	function __construct()
	{
		...
		{
			#sanitization
			$this->thisgroup     =  $db->real_escape_string($_REQUEST['thisgroup']);
			#check empty
			if(!empty($_REQUEST['chkArray']))
			{
				if($group_permit ==1)
				{
					...
					foreach ($_REQUEST['chkArray'] as $record) 
					{
						...
						$linkToGroup = $db->query("SELECT * FROM `sms_group_members` WHERE `member`='$record' && `group`='$this->thisgroup'");
