...
class New_Contact
{
	...
	function __construct()
	{
		...
		if(isset($_REQUEST['cname']) && isset($_REQUEST['corg']) && isset($_REQUEST['cphone']))
		{
			...
			{
				#sanitization
				$this->cname           =ucwords($db->real_escape_string($_REQUEST['cname']));
				$this->corg            =ucwords($db->real_escape_string($_REQUEST['corg']));
				$this->cphone          =$db->real_escape_string($_REQUEST['cphone']);
				...
				if(!empty($this->cname) && !empty($this->corg) && !empty($this->cphone))
				{
					...
					//verify phone number
							if(strpos($this->cphone,'+') !== false)
							{
								...
								$duplicate=$db->query("SELECT * FROM `sms_contacts` WHERE `phone_number`='$this->cphone'");

								if($duplicate->num_rows==0)
								{
									$insert = $db->query("INSERT INTO `sms_contacts`(`contact_name`,`organization`,`phone_number`,`added_by`,`date`)
