...
class SystemSettings
{
	...
	function __construct()
	{
		...
		if(isset($_REQUEST['twofactor']) && isset($_REQUEST['resettype']))
		{
			...
			{
				#sanitization
				$this->twofactor        =  $db->real_escape_string($_REQUEST['twofactor']);
				$this->resettype        =  ucwords($db->real_escape_string($_REQUEST['resettype']));
				#check empty
				if(!empty($this->resettype) && !empty($this->twofactor))
				{
					#check against database for record
						$update = $db->query("UPDATE `sms_settings` SET `security_2_factor`='$this->twofactor',`password_reset_type`='$this->resettype'");
							if($update)
								{
									# Write log
