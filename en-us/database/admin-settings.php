...
class AdminSettings
{
	...
	function __construct()
	{
		...
		{
				#sanitization
				$this->oldpassword        =  $db->real_escape_string($_REQUEST['oldpassword']);
				$this->newpassword        =  $db->real_escape_string($_REQUEST['newpassword']);
				$this->confirmnewpassword =  $db->real_escape_string($_REQUEST['confirmnewpassword']);
				#check empty
					...
					if(md5($this->oldpassword)==$pdata)
					{
						if($this->newpassword == $this->confirmnewpassword)
						{
							if(strlen($this->newpassword) < 8)
							{
								...
								$update = $db->query("UPDATE `sms_users` SET `password`='$enc' WHERE `id`='$uid'");
									if($update)
										{
											# Write log
