		...
		if(isset($_GET['groupreference']))
		{
			...
			if(!empty($_GET['groupreference']))
			{
				if($remove_group_contact ==1)
				{
					...
					#delete Records
					$deleteGroup = $db->query("DELETE FROM `sms_groups` WHERE `id`='$this->groupreference'");
					#delete from groups
					$deleteGContact = $db->query("DELETE FROM `sms_group_members` WHERE `group`='$this->groupreference'");
