...
class DeleteGContact
{
	...
	function __construct()
	{
		...
		{
			#sanitization
			$this->userReference     =  $db->real_escape_string($_GET['userReference']);
			$this->groupReference    =  $db->real_escape_string($_GET['groupReference']);
			#check empty
			if(!empty($_GET['groupReference']) && !empty($this->userReference))
			{
				if($remove_group_contact ==1)
				{
					...
					$deleteGContact = $db->query("DELETE FROM `sms_group_members` WHERE `member`='$this->userReference' && `group`='$this->groupReference'");
